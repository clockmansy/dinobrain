...
The current MVP has a policy-trusted managed Codex hook bridge plus the MCP memory system. The repository intentionally carries no duplicate DinoBrain project hook.

Expected states:

| Step | Current state | Meaning |
| --- | --- | --- |
| 1. User request detection | `verified` | The managed-hook installer exists, the repo policy rejects duplicate hooks, and the hook verifier simulates `os_begin_task` -> verified Context Pack -> `additionalContext`. |
...
| 7. Knowledge growth | `verified` | Explicit test cycles with `DINOBRAIN_AUTO_GROWTH=1` and `DINOBRAIN_AUTO_COMPOUND=1` prove reviewed growth and cleanup; installed prompt hooks keep both off by default. |
...
1. Fully restart Codex and open a fresh thread after managed requirements change. Use `/hooks` trust only when diagnostics say the user fallback is active.
